...
from SAR.sar_v1.simple_env_mod import SimpleEnv, make_env
...
class raw_env(SimpleEnv, EzPickle):
    def __init__(
        ...
        self.metadata["name"] = "simple_sar_v1"
...
class Scenario(BaseScenario):
    ...
    def make_world(self, num_agents=3, num_hostages=1, num_goals=2):
        ...
        fog_tiles = fog.create_tiles(1000, 100)



        world.fog_tiles = fog_tiles
        self.fog_tiles = fog_tiles
    ...
    def agent_reward(self, agent, world):
        ...
        # 12, 6
        # [0.16605937 0.33958836]

        for tile in self.fog_tiles:
            smaller_tile = [(tile.center[0])/div, (tile.center[1])/div]
            distance = self.heuristic(smaller_tile, agent.state.p_pos)

            if tile.left//tile.side == 12 and tile.top//tile.side == 6:
                print(f"for i : {tile.left//tile.side}")
                print(f"for j : {tile.top//tile.side}")
        ...
        if agent.collide:
            ...
            for gl in goals:
                if self.is_collision(agent, gl) and agent.active:
                    ...
                    rew += 300
                    self.reset_world(world, np_random=np.random)
        ...
        for entity in world.landmarks:
            if not entity.boundary:
                if entity.name == target:
                    ...
                    distance = self.heuristic(entity.state.p_pos, agent.state.p_pos)
                    #print(f"distance {distance}")
                    #print(f"distance_x {entity.state.p_pos[0] - agent.state.p_pos[0]}")
                    #print(f"distance_y {entity.state.p_pos[1] - agent.state.p_pos[1]}")


                    # pettingzoo pain
                    #if distance < 0.35:
                    #    #print(f" saw {entity.name}")
                    #    entity_list.append(entity)
                    #    entity_pos.append(entity.state.p_pos)
                    #else:
                    #    entity_pos.append([999, 999])
                    ...
                    #      f" at {entity.state.p_pos - agent.state.p_pos}"
